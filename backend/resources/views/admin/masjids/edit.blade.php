...
<section class="panel"><div class="panel-body">
    <form method="POST" enctype="multipart/form-data" action="{{ route('admin.masjids.update', $masjid) }}">@csrf @method('PUT')
        <div class="form-grid">
            ...
            <div class="field"><label for="countdown_beep_seconds">Countdown beeps before azan &amp; qamat (seconds)</label><input id="countdown_beep_seconds" name="countdown_beep_seconds" type="number" min="0" max="60" value="{{ old('countdown_beep_seconds', $masjid->countdown_beep_seconds ?? 10) }}" required><small>Beeps once per second for this many seconds before each azan and qamat. 10 = beeps 10 times.</small></div>
            ...
            <div class="field"><label for="language">Language</label><select id="language" name="language" required><option value="ms" @selected(old('language', $masjid->language ?: 'ms') === 'ms')>Bahasa Melayu</option><option value="en" @selected(old('language', $masjid->language) === 'en')>English</option></select><small>Controls the day and month shown on the TV screen (e.g. Selasa/August).</small></div>
            ...
            <div class="field full"><label>Fullscreen views to rotate</label><div style="display:grid;grid-template-columns:repeat(4,minmax(140px,1fr));gap:12px">
                @foreach(['main' => 'Utama', 'clock' => 'Clock', 'announcements' => 'Pengumuman', 'schedule' => 'Jadual Solat', 'donation' => 'Sumbangan', 'slides' => 'Maklumat', 'gallery' => 'Galeri'] as $value => $label)
                    <label style="display:flex;align-items:center;gap:8px"><input type="checkbox" name="rotation_views[]" value="{{ $value }}" style="width:auto" @checked(in_array($value, old('rotation_views', array_filter($masjid->rotation_views ?? ['main', 'clock', 'announcements', 'schedule', 'donation']))))>{{ $label }}</label>
                @endforeach
            </div><small>Each enabled view fills the screen for the duration below, then slides to the next. The main view stays at least 1 minute. Maklumat and Galeri only appear when there is content to show.</small></div>
            <div class="field"><label for="rotation_duration_seconds">Seconds per fullscreen view</label><input id="rotation_duration_seconds" name="rotation_duration_seconds" type="number" min="1" max="3600" value="{{ old('rotation_duration_seconds', $masjid->rotation_duration_seconds ?? 180) }}" required><small>How long each view stays before sliding to the next. 180 = 3 minutes. The main view stays at least 1 minute.</small></div>
            ...
            <div class="field"><label for="rotation_after_prayer_minutes">Start rotating after solat (minutes)</label><input id="rotation_after_prayer_minutes" name="rotation_after_prayer_minutes" type="number" min="0" max="180" value="{{ old('rotation_after_prayer_minutes', $masjid->rotation_after_prayer_minutes ?? 15) }}" required><small>The main dashboard keeps showing for this long after prayer ends before fullscreen views start sliding. 15 is recommended.</small></div>
