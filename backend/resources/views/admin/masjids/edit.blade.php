    ...
    <form method="POST" enctype="multipart/form-data" action="{{ route('admin.masjids.update', $masjid) }}">@csrf @method('PUT')
        <div class="form-grid">
            ...
            <div class="field full"><label>Fullscreen views to rotate</label><div style="display:grid;grid-template-columns:repeat(4,minmax(140px,1fr));gap:12px">
                @foreach(['clock' => 'Clock', 'announcements' => 'Pengumuman', 'schedule' => 'Jadual Solat', 'donation' => 'Sumbangan', 'slides' => 'Maklumat', 'gallery' => 'Galeri'] as $value => $label)
                    <label style="display:flex;align-items:center;gap:8px"><input type="checkbox" name="rotation_views[]" value="{{ $value }}" style="width:auto" @checked(in_array($value, old('rotation_views', array_filter($masjid->rotation_views ?? []))))>{{ $label }}</label>
                @endforeach
            </div><small>Each enabled view fills the screen for the duration below, then slides to the next. Maklumat and Galeri only appear when there is content to show.</small></div>
            <div class="field"><label for="rotation_duration_seconds">Seconds per fullscreen view</label><input id="rotation_duration_seconds" name="rotation_duration_seconds" type="number" min="1" max="3600" value="{{ old('rotation_duration_seconds', $masjid->rotation_duration_seconds ?? 180) }}" required><small>How long each view stays before sliding to the next. 180 = 3 minutes.</small></div>
